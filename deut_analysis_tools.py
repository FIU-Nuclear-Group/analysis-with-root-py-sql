    ...
    make_plots(v,branches,runs_list,apply_cuts=True,custom_cuts=None)
            ...
            apply_cuts: bool = True(default);
                        ...
                        standard cuts applied by default are:
                            ...
                            -10 < hms delta < 10
                            -3 < z_react_diff < 3
                            -0.03 < shms xptar < 0.03 
                            -0.03 < shms yptar < 0.03 
                            -0.08 < hms xptar < 0.08 
                            -0.03 < hms yptar < 0.03 
...
class make_plots:
    def __init__(self,v,branches,runs_list,apply_cuts=True,custom_cuts={}):
        ...
        # declare variable(s)
        #self.v = load_branches(self.runs_list,self.v,self.branches)
...
#                 H_dp = self.branches[run]['H.gtr.dp']
#                 P_dp = self.branches[run]['P.gtr.dp']
#                 H_react_z = self.branches[run]['H.react.z']
#                 P_react_z = self.branches[run]['P.react.z']
#                 P_xptar = self.branches[run]['P.gtr.th']
#                 P_yptar = self.branches[run]['P.gtr.ph']
#                 H_xptar = self.branches[run]['H.gtr.th']
#                 H_yptar = self.branches[run]['H.gtr.ph']
#                   
#                 # cut variables operations
#                 z_diff = abs(H_react_z-P_react_z)
#     
#                 # define default cuts
#                 H_dp_cut_max = H_dp < 15.
#                 H_dp_cut_min = H_dp > -15.
#                 H_dp_cut = H_dp_cut_min & H_dp_cut_max
#     
#                 P_dp_cut_max = P_dp < 25.
#                 P_dp_cut_min = P_dp > -15.
#                 P_dp_cut = P_dp_cut_min & P_dp_cut_max
#     
#                 z_diff_cut_max = z_diff < 3.
#                 z_diff_cut_min = z_diff > -3.
#                 z_diff_cut = z_diff_cut_min & z_diff_cut_max
#                 
#                 P_xptar_cut_max= P_xptar < 0.03
#                 P_xptar_cut_min= P_xptar > -0.03
#                 P_xptar_cut = P_xptar_cut_max & P_xptar_cut_min 
# 
#                 P_yptar_cut_max= P_yptar < 0.03
#                 P_yptar_cut_min= P_yptar > -0.03
#                 P_yptar_cut = P_yptar_cut_max & P_yptar_cut_min 
#                 
#                 H_xptar_cut_max= H_xptar < 0.08
#                 H_xptar_cut_min= H_xptar > -0.08
#                 H_xptar_cut = H_xptar_cut_max & H_xptar_cut_min 
# 
#                 H_yptar_cut_max= H_yptar < 0.03
#                 H_yptar_cut_min= H_yptar > -0.03
#                 H_yptar_cut = H_yptar_cut_max & H_yptar_cut_min 
#                     
#             # select cuts to apply
#             for c_var in self.custom_cuts:
#                 for run in self.runs_list:
#                     self.cuts[run] = self.c_cuts[c_var][run] & H_dp_cut & P_dp_cut\
#                         & z_diff_cut & P_xptar_cut & P_yptar_cut & H_xptar_cut & H_yptar_cut 
# =============================================================================
...
# =============================================================================
    ...
    def make_1Dhistos(self,histo_params=[(-100,100),100],fig_title=None,
                      apply_norm=False,SIMC=None,plot=True):
        ...
        for var in self.v.keys():
            ...
            for runs in self.runs_list:                
                ...
                if apply_norm:
                    h = self.norm*B.histo(v,histo_params[0],histo_params[1],
                ...
                if self.SIMC != None:
                    hS = self.SIMC.make_1Dhistos(histo_params=self.histo_params,
    ...
    def make_2Dhistos(self,histo_params=[(-100,100),100,(-100,100),100],
                      fig_title='',apply_norm=False,plot=True):
        self.fig_title = fig_title
        # plot variable(s)
        if plot:
            fig = plt.figure(layout='constrained')
...
class make_plots_SIMC:
    def __init__(self,v,branches,apply_cuts=True,custom_cuts=None):
        self.v = dict.fromkeys(cast_to_list(v))     # variables to plot
        self.branches = branches                    # dict where the loaded 
                                                    #  variables are located
        self.custom_cuts = custom_cuts              # add more cuts
        ...
        if apply_cuts:
            # cuts = {}
            # define cut variables
            H_dp = self.branches['h_delta']
            P_dp = self.branches['e_delta']
            react_z = self.branches['tar_z']
            P_xptar = self.branches['e_xptar']
            P_yptar = self.branches['e_yptar']
            H_xptar = self.branches['h_xptar']
            H_yptar = self.branches['h_yptar']
            ...
            z_diff_cut_max = react_z < 3.
            z_diff_cut_min = react_z > -3.
            z_diff_cut = z_diff_cut_min & z_diff_cut_max
            ...
            # selection of cuts
            cuts = H_dp_cut & P_dp_cut & z_diff_cut & P_xptar_cut &\
                    P_yptar_cut & H_xptar_cut & H_yptar_cut

            # apply cuts to variable(s)
            for var in self.v.keys():
                cut_var = self.v[var][cuts]
                self.v[var] = cut_var
                
            n = self.calc_weights()   
            self.totweights = n[cuts]
        ...
        for var in self.v.keys():
            ...
            if apply_weights:
                hS = B.histo(v,histo_params[0],histo_params[1],
                            weights=self.totweights,density=False,
                            calc_w2=True)
            else:
                hS = B.histo(v,histo_params[0],histo_params[1])
    ...
    def make_2Dhistos(self,histo_params=[(-100,100),100,(-100,100),100],
        ...
        if apply_weights:
            h = B.histo2d(vx,vy,[histo_params[0],histo_params[2]],
                      [histo_params[1],histo_params[3]],logz=True,
                      title='', xlabel='', ylabel='', weights=self.totweights,
                      calc_w2=True)
        else:
            h = B.histo2d(vx,vy,[histo_params[0],histo_params[2]],
                          [histo_params[1],histo_params[3]],logz=True,
...
class DEUT_DATA:
    def __init__(self,kin_study=None,setting=None,branches_sel=[],Runs=None,
                 load=True):
    ...
    def TREES(self,r,root,p):
        self.Trees = {}
        
        for run_num in r:
            rootfile = p + root[run_num].file
            if os.path.isfile(rootfile) :
                self.Trees[run_num]= R.root_tree(rootfile, trees=['T','TSP'])
...
# DATA ========================================================================
pm120 = DEUT_DATA(kin_study='deep',setting='pm_120',branches_sel=['kins'])
...
# # SHMS Beta
# P_beta = plot_var('P.hod.beta',ana_branches,ana_runs,(2,5),
#                   fig_title='SHMS Beta (no cuts)')
# P_beta.histo1D(histo_params=[(0.5,1.5),100])
# # SHMS Beta no track
# P_beta_notrack = plot_var('P.hod.betanotrack',ana_branches,ana_runs,(2,5),
#                   fig_title='SHMS Beta (no track, no cuts)')
# P_beta_notrack.histo1D(histo_params=[(0.5,1.5),100])

# # HMS Beta
# H_beta = plot_var('H.hod.beta',ana_branches,ana_runs,(2,5),
#                   fig_title='HMS Beta (no cuts)')
# H_beta.histo1D(histo_params=[(0.5,1.5),100])

# # HMS Beta no track
# H_beta_notrack = plot_var('H.hod.betanotrack',ana_branches,ana_runs,(2,5),
#                   fig_title='HMS Beta (no track, no cuts)')
# H_beta_notrack.histo1D(histo_params=[(0.5,1.5),100])

# # SHMS Beta vs x_fp
# P_beta = plot_var(['P.dc.x_fp','P.hod.beta'],ana_branches,ana_runs,(2,5),
#                   fig_title='SHMS Beta vs. x_fp (no cuts)')
# P_beta.histo2D(histo_params=[(-40,23),100,(0.5,1.5),100])

# # SHMS Beta vs y_fp
# P_beta = plot_var(['P.dc.y_fp','P.hod.beta'],ana_branches,ana_runs,(2,5),
#                   fig_title='SHMS Beta vs. y_fp (no cuts)')
# P_beta.histo2D(histo_params=[(-40,23),100,(0.5,1.5),100])

# # SHMS Beta vs delta
# P_beta = plot_var(['P.gtr.dp','P.hod.beta'],ana_branches,ana_runs,(2,5),
#                   fig_title='SHMS Beta vs. delta (no cuts)')
# P_beta.histo2D(histo_params=[(-25,15),100,(0.5,1.5),100])

# # HMS Beta vs x_fp
# H_beta = plot_var(['H.dc.x_fp','H.hod.beta'],ana_branches,ana_runs,(2,5),
#                   fig_title='HMS Beta vs. x_fp (no cuts)')
# H_beta.histo2D(histo_params=[(-40,23),100,(0.5,1.5),100])

# # HMS Beta vs y_fp
# H_beta = plot_var(['H.dc.y_fp','H.hod.beta'],ana_branches,ana_runs,(2,5),
#                   fig_title='HMS Beta vs. y_fp (no cuts)')
# H_beta.histo2D(histo_params=[(-40,23),100,(0.5,1.5),100])

# # HMS Beta vs delta
# H_beta = plot_var(['H.gtr.dp','H.hod.beta'],ana_branches,ana_runs,(2,5),
#                   fig_title='HMS Beta vs. delta (no cuts)')
# H_beta.histo2D(histo_params=[(-25,15),100,(0.5,1.5),100])
